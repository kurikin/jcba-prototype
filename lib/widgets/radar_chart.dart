...
class RadarChart extends StatefulWidget {
  ...
  const RadarChart({
    ...
    this.outlineColor = Colors.black54,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    features.asMap().forEach((index, feature) {
      ...
      TextPainter(
        ...
        ..paint(
            canvas,
            Offset(-3 + labelXOffset * 1.02,
                featureOffset.dy * 1.01 + labelYOffset * 1.57));
    });
    ...
    data.asMap().forEach((index, graph) {
      var graphPaint = Paint()
        ..color = index == 1
            ? graphColors[index % graphColors.length].withOpacity(0.6)
            : graphColors[index % graphColors.length].withOpacity(0.05)
